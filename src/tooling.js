...
// All the preview / sync / inline editing tooling that requires Node.js
// This file is NOT safe for browser consumption.
...
export async function syncPreviewArtifact(config) {
  ...
  if (!config.endpoint) return artifact;
  ...
  return { artifact, response: await response.json() };
}
...
export async function createWordVelPreview(config) {
  ...
  if (typeof config.renderPreviewBlocks !== 'function') {
    throw new Error('wordvel-typescript sync now expects config.renderPreviewBlocks. Use @wordvel/react for React component configs.');
  }

  const renderedBlocks = await config.renderPreviewBlocks({ manifest, placeholders, config });

  return createPreviewArtifact({ blocks: renderedBlocks, css });
}

export async function syncWordVelPreview(config) {
  const artifact = await createWordVelPreview(config);
  return syncPreviewArtifact({ artifact, endpoint: config.endpoint, headers: config.headers });
}
...
export function createInlineEditBinding(field) {
  ...
  return {
    field: { path: field.path, kind, label: field.label, required: Boolean(field.required) },
    attributes: {
...
export function createInlineEditPatch(input) {
  if (!input.blockClientId) throw new Error('Inline edit patches require a blockClientId.');
  if (!input.path) throw new Error('Inline edit patches require a field path.');
  return { blockClientId: input.blockClientId, path: input.path, value: input.value };
}

export function getValueAtPath(data, path) {
  return pathSegments(path).reduce((value, segment) => (value == null ? undefined : value[segment]), data);
}
...
// --- private helpers (moved from old index.js) ---
...
function placeholderValue(field, prefix) {
  ...
  if (field.type === 'repeater') {
    ...
    return Array.from({ length: itemCount }, (_, index) =>
      Object.fromEntries((field.fields ?? []).map((nestedField) => [
        nestedField.key,
  ...
  if (field.type === 'localized_text') return `{{ ${prefix}${field.key} }}`;
  if (field.type === 'image' || field.type === 'media') {
    return { id: '', url: `{{ ${prefix}${field.key}.url }}`, alt: `{{ ${prefix}${field.key}.alt }}`, width: '', height: '' };
  }
  if (field.type === 'number') return 0;
  if (field.type === 'boolean' || field.type === 'toggle') return false;
  if (field.type === 'select') return firstOptionKey(field.options?.choices ?? field.options) ?? `{{ ${prefix}${field.key} }}`;
  if (field.type === 'url' || field.type === 'link') return '#';
  if (field.type === 'icon') return firstOptionKey(field.options?.icons ?? field.options) ?? '{{ icon }}';
...
function normalizeInlineEditKind(kind) {
  const normalizedKinds = {
    text: 'text', string: 'text', textarea: 'text',
    rich_text: 'richText', richText: 'richText', wysiwyg: 'richText',
    url: 'url', link: 'url',
    number: 'number', boolean: 'boolean', toggle: 'boolean', select: 'select',
  };
  const normalized = normalizedKinds[kind];
  if (!normalized) throw new Error(`Unsupported inline editable field kind: ${kind}`);
  return normalized;
}
...
function setAtSegment(data, segments, value) {
  ...
  const next = Array.isArray(data) ? [...data] : data == null && typeof segment === 'number' ? [] : { ...(data ?? {}) };
  if (rest.length === 0) {
    next[segment] = value;
    return next;
  }
